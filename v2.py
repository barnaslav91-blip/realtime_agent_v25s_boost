...
# Trailing stop на основе MedianRange
TRAIL_START_MULT = 1.5
TRAIL_OFFSET_MULT = 1.0
TRAIL_BREAK_EVEN_MULT = 0.2
...
def init_mt5():
    ...
    if symbol_info is None:
        raise RuntimeError(f"Символ {SYMBOL} не найден. Добаь его в Market Watch.")
...
def close_position(ticket: int, direction: int, volume: float):
    ...
    if result is None or result.retcode != mt5.TRADE_RETCODE_DONE:
        print(f"[CLOSE] Ошибка закрытия, result={result}")
        return None

    print(f"[CLOSE] Позиция закрыта ticket={ticket} по цене={price}")
    return result

def update_trailing_sl(ticket: int, direction: int, entry_price: float, current_sl: float, tp_price: float, median_range: float):
    """Обновляем SL при достаточном профите."""
    if ticket is None or median_range <= 0:
        return None

    tick = mt5.symbol_info_tick(SYMBOL)
    if tick is None:
        print("[TRAIL] Не удалось плучить tick.")
        return None

    price = tick.bid if direction == 1 else tick.ask
    profit_distance = (price - entry_price) if direction == 1 else (entry_price - price)
    trail_start = median_range * TRAIL_START_MULT
    if profit_distance < trail_start:
        return None

    offset = median_range * TRAIL_OFFSET_MULT
    breakeven_shift = median_range * TRAIL_BREAK_EVEN_MULT
    if direction == 1:
        desired_sl = price - offset
        desired_sl = max(desired_sl, entry_price + breakeven_shift)
        if current_sl is not None and desired_sl <= current_sl:
            return None
    else:
        desired_sl = price + offset
        desired_sl = min(desired_sl, entry_price - breakeven_shift)
        if current_sl is not None and desired_sl >= current_sl:
            return None
    ...
    result = mt5.order_send(request)
    if result is None or result.retcode != mt5.TRADE_RETCODE_DONE:
        print(f"[TRAIL] Ошибка обновления SL, result={result}")
        return None

    print(f"[TRAIL] SL обновлён ticket={ticket}, new_sl={desired_sl:.5f}")
    return result
...
def run_agent():
    ...
    while True:
        ...
        if pos_dir != 0:
            if last_trade_bar_index is None:
                last_trade_bar_index = len(df) - 1

            bars_held = (len(df) - 1) - last_trade_bar_index
            if bars_held >= MAX_HOLD_BARS:
                print(f"[TIME_EXIT] Позиция держится {bars_held} баров, закрываем.")
                close_position(pos_ticket, pos_dir, pos_vol)
                time.sleep(5)
                continue

            update_trailing_sl(pos_ticket, pos_dir, pos_entry, pos_sl, pos_tp, median_range)
        ...
        macd_hist = float(row_sig["MACD_HIST"]) if not pd.isna(row_sig["MACD_HIST"]) else 0.0
        ...
        # 3) MACD-фильтр — подтверждаем направление гистограммой
        if signal == "BUY" and macd_hist < MACD_MIN_HIST:
            print(f"[MACD] BUY отклонён, hist={macd_hist:.5f}.")
            time.sleep(5)
            continue
        if signal == "SELL" and macd_hist > -MACD_MIN_HIST:
            print(f"[MACD] SELL отклонён, hist={macd_hist:.5f}.")
            time.sleep(5)
            continue
        ...
        if last_trade_bar_index is not None:
            bars_since_trade = (len(df) - 1) - last_trade_bar_index
            if bars_since_trade < MIN_BARS_BETWEEN_TRADES:
                print(f"[COOLDOWN] До следующей сделки нужно {MIN_BARS_BETWEEN_TRADES - bars_since_trade} баров.")
                time.sleep(5)
                continue
